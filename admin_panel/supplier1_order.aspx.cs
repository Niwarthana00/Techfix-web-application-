...
{
    public partial class supplier1_order : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                LoadCategories();
                LoadProductsByCategory(1); // Load default category (category_id = 1) on page load
        ...
        public class Product
        {
            ...
            // Total price calculated property
        ...
        {
            string query = "SELECT id, product_name, image, price, description, qty FROM s1product WHERE category_id = @CategoryId";
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
        ...
        {
            Response.Redirect("s1place_order.aspx");
        }
