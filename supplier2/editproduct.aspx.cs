...
namespace techfix.supplier2
{
    public partial class editproduct : System.Web.UI.Page
    {
        ...
        private void LoadProductDetails(int productId)
        {
            ...
            string query = "SELECT product_name, price, description, qty, image FROM s2product WHERE id = @id";
        ...
        {
            string connectionString = WebConfigurationManager.ConnectionStrings["techfixdbConnectionString"].ConnectionString;
            string query = "UPDATE s2product SET product_name = @productName, price = @price, description = @description, qty = @qty, image = @image WHERE id = @id";
